...
class LkpAdaptUtil:
    ...
    def get_regional(self):
        adaptations = self.db.query(LkpAdapt).filter(
            and_(
                LkpAdapt.commodity_type_id == None,
                LkpAdapt.commodity_id == None  
            )
        ).order_by(LkpAdapt.id).all()
        options_list = [{
    ...
    def get_data(self):
        if self.commodity_id:
            # Commodity specific options - either crops or livestock
            ...
            # Regional analysis options -- without commodity selection
            groups_queryset = self.db.query(LkpAdaptGroup).filter(LkpAdaptGroup.commodity_type_id == None).all()
            self.groups = {i.id: i.group for i in groups_queryset}
            return self.get_regional()
